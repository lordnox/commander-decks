...
export type ConduitFrame = {
  kind: ConduitKind
  generation: number
  body: Uint8Array
}

export const decodePrefix = (bytes: Uint8Array): ConduitFrame | null => {
...
const asBytes = async (data: unknown, acceptStringMessages: boolean) => {
  if (data instanceof ArrayBuffer) return new Uint8Array(data)
  if (ArrayBuffer.isView(data)) {
    return new Uint8Array(data.buffer, data.byteOffset, data.byteLength)
  }
  if (data instanceof Blob) return new Uint8Array(await data.arrayBuffer())
  if (acceptStringMessages && typeof data === 'string') {
    return new TextEncoder().encode(data)
  }
  return null
}

export type WatchSnapshotsOptions = {
  from?: number
  onStatus?: (connected: boolean) => void
  acceptStringMessages?: boolean
}

export const watchSnapshots = (
  origin: string,
  readKey: string,
  onSnapshot: (frame: ConduitFrame) => void,
  options: WatchSnapshotsOptions = {},
) => {
  ...
  const acceptStringMessages = options.acceptStringMessages ?? false
  ...
  const connect = () => {
    ...
    socket.addEventListener('message', (event) => {
      void asBytes(event.data, acceptStringMessages).then((bytes) => {
