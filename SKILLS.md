...
The skill is a discussion: ask only open questions, verify Oracle and current Game Changers, and do not fill in guesses. It challenges commander choice, named finishers, loop math (including what Rings can copy), how finishers are found (draw vs tutors), constraints (root `DECISIONS.md`: staples are flour, so avoid the identity-diluting ones while lands and two-mana rocks stay exempt, esoteric on-theme cards as a tiebreaker, discussable Game Changers), the [interaction package](INTERACTION.md) by purpose rather than a raw count, the [mana plan](MANABASE.md), the [construction skeleton](CONSTRUCTION.md) (ramp/draw mix and package density as hints), and the brew title. It does not write `decklist.txt` until that plan is locked. If the user already posted a complete list, skip this skill and use `deck-workspace`.
