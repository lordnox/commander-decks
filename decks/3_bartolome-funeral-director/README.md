...
The deck uses that ability in three ways:
...
The usual loop needs:

- Bartolomé on the battlefield;
- a creature that can return the relevant Aura or permanent from the graveyard;
...
- Develop mana rocks.
- Cast self-mill creatures and spells.
- Put recursion pieces into the graveyard.
- Avoid exposing a payoff before it is useful.
- Cast Bartolomé when you have something valuable to sacrifice or need protection from exile.

### Midgame

- Find a return effect and a creature that can recover it.
- Use Bartolomé to sacrifice creatures in response to exile, theft, or unfavorable removal.
- Recur utility cards and removal while assembling the engine.
- Keep enough mana or graveyard resources to rebuild if the first attempt is stopped.
...
- two or three lands;
- a mana rock or cheap setup spell;
- a self-mill, draw, or graveyard-recursion card; and
- access to both colors.

Keep a combo piece when the rest of the hand functions. Ship hands containing several expensive recursion creatures but no ramp, mill, or early play.
...
- Bartolomé's ability costs no mana and can be activated at instant speed.
- Sacrifice threatened creatures instead of allowing opponents to exile or steal them.
- With several death triggers, stack the return trigger and payoff triggers deliberately.
- Do not sacrifice the only recursion piece until its return effect is active.
- Check whether the returning creature can recover the Aura or permanent immediately; delayed-return cards do not form the same loop.
- Graveyard hate is the deck's biggest weakness. Hold removal for effects that exile graveyards or replacement effects that prevent cards from reaching them.
...
Priority order usually is:

1. The recursion creature that closes the loop.
2. The return effect attached to it.
3. The payoff that converts the loop into a win.
4. Bartolomé, when commander tax would prevent the attempt.

Because Bartolomé is cheap and always available from the command zone, it is often correct to protect the harder-to-replace engine piece instead.
