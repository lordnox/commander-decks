...
- **Untapped fixing over filler tap lands.** Dual lands are welcome and are not excluded for price unless a deck has a budget. An always-tapped land needs meaningful deck-specific upside beyond incidental life gain; otherwise use an untapped basic or dual. Follow [MANABASE.md](MANABASE.md) for source, tempo, and utility checks.
...
- **2026-09-13 — Mana bases are deck slots.** Build fixing and source counts first, reject tap lands without real upside, then use spare land slots as synergistic utility. Dual lands are allowed by default; budget is a per-deck constraint, not a repository assumption.
