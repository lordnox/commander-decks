...
- Every entry comes from the deck directory prefix (bracket badge), `rankings.json` when present (a `J# F# M#` code plus identity-goal badges immediately after the bracket), the primer H1 or the optional `title` in `tags.json` (label), and `tags.json` (summary and tag links). A deck without a valid `tags.json` never reaches the index.
