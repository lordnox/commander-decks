...
- Every entry comes from the deck directory prefix (bracket badge), the primer H1 or the optional `title` in `tags.json` (label), and `tags.json` (summary and badges); ranking badges stay on the primer. A deck without a valid `tags.json` never reaches the index.
