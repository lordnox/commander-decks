...
The primer shows one shields.io badge per score, `Jank`, `Fun`, `Mean`, then identity goals in `goals` order, in a row after the Archidekt tag badges. Badge color darkens with the score: teal where a high score is desirable, red for `Mean`, where a high score is the thing to watch. Rankings stay on the primer; the root README index deliberately shows only bracket, summary, and tags. Primer refresh also runs this skill; if goals are missing, skip scoring rather than inventing them.
