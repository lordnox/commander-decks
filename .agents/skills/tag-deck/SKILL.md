...
The index is generated, so never edit it by hand. It groups decks under `### Bracket N · Name` headings, sorts them by bracket, then `−` / plain / `+`, then title, and shows each deck's bracket, its `J# F# M#` ranking code and identity-goal badges from `rankings.json` when present, the summary, plus its top three tag names as small links with a `+N more` link to the primer. Root-index tags carry no score badge; the primer keeps the scored badges, whose color darkens with the score. Every deck needs a valid `tags.json`, or it silently drops out of the index.
