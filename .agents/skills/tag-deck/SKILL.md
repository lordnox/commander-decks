...
The index is generated, so never edit it by hand. It is one comparison table with a row per deck, sorted by kitchen-table power in root `power-rankings.json` (see [POWER-RANKINGS.md](../../../POWER-RANKINGS.md)), not by bracket. Columns are the linked deck title, bracket, the Jank, Fun, and Mean scores from `rankings.json` (rendered by `deck_rankings.score_cell`, which colors the extremes), and that deck's identity goals as smaller badges; decks without rankings show `—`. Summaries and scored tag badges live on the primers only. Every deck still needs a valid `tags.json`, or it silently drops out of the index. A new deck must be appended to `power-rankings.json` (bottom until placed) so `--check` succeeds.
