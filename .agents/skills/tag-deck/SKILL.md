...
The index is generated, so never edit it by hand. It groups decks under `### Bracket N · Name` headings, sorts them by bracket, then `−` / plain / `+`, then title, and shows each deck's bracket, ranking badges from `rankings.json` when present (Jank, Fun, Mean, then identity goals), summary, plus its top four tag badges with a `+N more` badge linking to the primer. Badge color darkens with the score. Every deck needs a valid `tags.json`, or it silently drops out of the index.
