...
Badges sit immediately after the Archidekt sandbox link. The same run regenerates the deck comparison table in the root README, where the deck gets one row with its bracket and its ranking scores when `rankings.json` exists. `--check` must succeed. Card categories stay separate from these deck tags.
